...
/**
 * @brief A virtual interface which can be reimplemented to receive roster
 * updates.
 *
 * A class implementing this interface and being registered as RosterListener
 * with the RosterManager object receives notifications about all the changes in
 * the server-side roster. Only one RosterListener per Roster at a time is
 * possible.
 *
...
class GLOOX_API RosterListener {
 public:
   ...
   * were removed from the server-side roster (items subject to a so-called
   * Roster Push). This function will be called regardless who deleted the item,
   * either this resource or another.
  ...
  /**
   * Reimplement this function if you want to be notified about items that
   * were modified on the server-side roster (items subject to a so-called
   * Roster Push). A roster push is initiated if a second resource of this JID
   * modifies an item stored on the server-side contact list. This can include
   * modifying the item's name, its groups, or the subscription status. These
   * changes are pushed by the server to @b all connected resources. This is why
   * this function will be called if you modify a roster item locally and
   ...
   * RosterManager asks the server for the full server-side roster. Invocation
   * of this method announces its arrival. Roster item status is set to
   * 'unavailable' until incoming presence info updates it. A full roster push
  ...
  /**
   * This function is called on every status change of a JID that matches the
   * Client's own JID. If the presence is of type Unavailable, then the resource
   * has already been removed from the RosterItem.
  ...
  /**
   * This function is called when an entity wishes to subscribe to this entity's
   * presence. If the handler is registered as a asynchronous handler for
   * subscription requests, the return value of this function is ignored. In
   * this case you should use RosterManager::ackSubscriptionRequest() to answer
   * the request.
   * @param jid The requesting item's address.
   * @param msg A message sent along with the request.
   * @return Return @b true to allow subscription and subscribe to the remote
   * entity's presence, @b false to ignore the request.
   */
  virtual bool handleSubscriptionRequest(const JID& jid,
                                         const std::string& msg) = 0;

  /**
   * This function is called when an entity unsubscribes from this entity's
   * presence. If the handler is registered as a asynchronous handler for
   * subscription requests, the return value of this function is ignored. In
   * this case you should use RosterManager::unsubscribe() if you want to
   * unsubscribe yourself from the contct's presence and to remove the contact
   * from the roster.
   * @param jid The item's address.
   * @param msg A message sent along with the request.
   * @return Return @b true to unsubscribe from the remote entity, @b false to
   * ignore.
   */
  ...
  /**
   * This function is called whenever presence from an entity is received which
  ...
  /**
   * This function is called for incoming Roster Item Exchange (@xep{0144})
   * suggestions. See RosterX.
   * @param from The entity suggesting the Roster Item exchange.
   * @param items The suggested item(s). Check the items' action() function to
   * find out whether this is a suggested addition/deletion/modification.
   * @since 1.1
   */
  virtual void handleRosterItemExchange(const JID& from,
                                        const RosterX* items) = 0;
#endif  // GLOOX_MINIMAL
};
