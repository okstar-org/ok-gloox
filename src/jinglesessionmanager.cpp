...
namespace gloox
{
  ...
  namespace Jingle
  {
    ...
    Session* SessionManager::createSession( const JID& callee, SessionHandler* handler, std::string sId)
    {
      if( !( handler || m_handler ) || !callee )
        return 0;

      Session* sess = new Session( m_parent, callee, handler ? handler : m_handler, sId  );
