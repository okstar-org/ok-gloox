...
namespace gloox
{
    ...
    : m_type( TypeInvalid ), m_required( false ), m_media(0)
  {
    ...
    for( ; it != l.end(); ++it )
    {
      ...
      else if( (*it)->name() == "media" && (*it)->xmlns() == XMLNS_DATAFORM_MEDIA )
      {
        m_media = new DataFormMedia( (*it ) );
      }
    }
